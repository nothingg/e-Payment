...
namespace AnyID.Models
{
    public class RegisterModel
    {
        ...
        public static string GetAcctClsDesc(string AcctCls) 
        {
            switch (AcctCls)
            {
                case "N":
        ...
        public static bool LogsInsert(DateTime dt , ANYID_DATA_CBS rsCBS , ANYID_DATA_ITMX rsITMX)
        {
            try
            {
                ...
                if (rsCBS != null) {
                    ...
                    rsLog.LOG_DATA = JsonConvert.SerializeObject(rsCBS);
                }
                if(rsITMX != null) {
                    rsLog.LOG_ACTION = "INSERT";
                    rsLog.LOG_DATA = JsonConvert.SerializeObject(rsITMX);
                }

                rsLog.CREATED_DATE = dt;
                rsLog.CREATED_USER = Convert.ToString(HttpContext.Current.Session["UserID"]);  /* ดึงจาก Session*/
                rsLog.CREATED_SOL = Convert.ToString(HttpContext.Current.Session["SolID"]);  /* ดึงจาก Session*/
